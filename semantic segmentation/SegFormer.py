...
class SegFormerEncoderBlock(nn.Sequential):
    def __init__(self, channels: int, reduction_ratio: int = 1, num_heads: int = 8, mlp_expansion: int = 4, drop_path_prob: float = 0.0):
        super().__init__(
            ResidualAdd(
                nn.Sequential(
                    LayerNorm2d(channels),
            ...
            ResidualAdd(
                nn.Sequential(
                    LayerNorm2d(channels), MixMLP(channels, expansion=mlp_expansion), StochasticDepth(p=drop_path_prob, mode="batch")
                )
            ),
        )
...
class SegFormer(nn.Module):
    ...
    def forward(self, x):
        ...
        segmentation = nn.functional.interpolate(segmentation, size=x.shape[-1], mode="bilinear", align_corners=False)
        return segmentation
